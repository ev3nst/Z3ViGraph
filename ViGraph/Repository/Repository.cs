...
namespace ViGraph.Repository
{
	public abstract class Repository<T, TDTO> : IRepository<T, TDTO> where T : class
	{
		...
		public abstract Task<DataTableResponse<TDTO>> Paginate(PaginationOptions PaginationOptions);

        public DataTableResponse<TDTO> ConfigureDataTableMeta(
            DataTableResponse<TDTO> dtResponse,
            PaginationOptions paginationOptions,
            string APIRoutePrefix
        ) {
            dtResponse.meta.from = (paginationOptions.PerPage * (paginationOptions.Page - 1)) + 1;
            dtResponse.meta.to = paginationOptions.PerPage * paginationOptions.Page;
            dtResponse.meta.total = dtResponse.recordsFiltered;

            if(dtResponse.pages > 1) {
                string[] linksArr = new string[dtResponse.pages];

                for (int i = 0; i < dtResponse.pages; i++)
                {
                    linksArr[i] = APIRoutePrefix + "?Page=" + i.ToString();
                }

                dtResponse.meta.links = linksArr;
                dtResponse.meta.first_page_url = linksArr[0];
                dtResponse.meta.last_page_url = linksArr[dtResponse.pages - 1];

                if(paginationOptions.Page < dtResponse.pages) {
                    dtResponse.meta.next_page_url = linksArr[paginationOptions.Page + 1];
                }

                if(paginationOptions.Page > 1) {
                    dtResponse.meta.prev_page_url = linksArr[paginationOptions.Page - 1];
                }
            } else {
                dtResponse.meta.links = new string[1] { APIRoutePrefix + "?Page=1" };
                dtResponse.meta.first_page_url = APIRoutePrefix + "?Page=1";
                // dtResponse.meta.last_page_url 
            }

            dtResponse.meta.page = paginationOptions.Page;
            dtResponse.meta.pages = dtResponse.pages;
            dtResponse.meta.last_page = dtResponse.pages;
            dtResponse.meta.perpage = paginationOptions.PerPage;

            dtResponse.meta.field = paginationOptions.SortField;
            dtResponse.meta.sort = paginationOptions.SortOrder.ToString().ToLower();
            dtResponse.meta.path = APIRoutePrefix;
